...
public class CabbageHudScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        MinecraftClient mc = this.client;
        if (mc != null && mc.player != null && button == 0) {
            CabbageConfig cfg = CabbageConfig.get();
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double offsetX, double offsetY) {
        if (draggingPanel) {
            movePanel((int) mouseX - panelDragOffX, (int) mouseY - panelDragOffY);
            return true;
        }
        if (dragging != null) {
            CabbageConfig.Elem st = CabbageConfig.get().elem(dragging.id, dragging.defX, dragging.defY);
            st.x = Math.max(0, Math.min(this.width - 4, (int) mouseX - dragOffX));
            st.y = Math.max(0, Math.min(this.height - 4, (int) mouseY - dragOffY));
            return true;
        }
